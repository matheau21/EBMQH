...
async function loadQuestion(id: string) {
  const { data: q, error } = await supabaseAdmin
    .from("questions")
    .select(
      "id, prompt, specialty, presentation_id, explanation, reference_url, highlights, is_active, created_at, updated_at",
    )
    .eq("id", id)
    .single();
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    let query = supabaseAdmin
      .from("questions")
      .select(
        "id, prompt, specialty, presentation_id, explanation, reference_url, highlights, is_active, status, created_at, updated_at",
      )
      .eq("is_active", true)
      .eq("status", "approved");
  ...
  async (req: AdminAuthRequest, res: Response) => {
    try {
      ...
      if (correctCount !== 1)
        return res
          .status(400)
          .json({ error: "Exactly one correct choice is required" });
      ...
      if (error || !q)
        return res
          .status(500)
          .json({ error: error?.message || "Failed to create" });
      ...
      return res
        .status(201)
        .json({ message: "Question created", question: loaded.question });
    } catch (err) {
  ...
  async (req: AdminAuthRequest, res: Response) => {
    try {
      ...
      if (body.specialty !== undefined)
        patch.specialty = body.specialty || null;
      if (body.presentationId !== undefined)
        patch.presentation_id = body.presentationId || null;
      if (body.explanation !== undefined)
        patch.explanation = body.explanation || null;
      if (body.referenceUrl !== undefined)
        patch.reference_url = body.referenceUrl || null;
      ...
      if (loaded.error || !loaded.question)
