  ...
  async (_req: AdminAuthRequest, res: Response) => {
    try {
      ...
      if (presErr || !presentations) {
        return res
          .status(500)
          .json({ error: presErr?.message || "Failed to fetch presentations" });
      }
      ...
      if (filesErr || !files) {
        return res
          .status(500)
          .json({ error: filesErr?.message || "Failed to list bucket files" });
      }

      // Parse filenames to match with presentations
      const matches: FileMatch[] = [];
      const filesByPresentation: {
        [key: string]: { pdfs: string[]; ppts: string[] };
      } = {};
