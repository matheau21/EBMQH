...
export default function AdminQuestions() {
  ...
  const filteredQuestions = useMemo(() => {
    ...
    return list.filter((q) => {
      if (
        filterPresentation !== "all" &&
        q.presentationId !== filterPresentation
      )
        return false;
      if (filterSpecialty !== "all") {
        const p = q.presentationId ? presMap[q.presentationId] : null;
        const presSpecs: string[] = Array.from(
          new Set(
            [p?.specialty, ...((p?.specialties as string[]) || [])].filter(
              Boolean,
            ),
          ),
        );
    ...
    mutationFn: () =>
      questionsAPI.create({
        ...
        highlights: (form.highlights || [])
          .filter((h) => h.phrase && h.page && h.page > 0)
          .map((h) => ({
            page: Number(h.page),
            phrase: h.phrase,
  ...
  const addChoice = () => {
    setForm((prev) => ({
      ...prev,
      choices:
        prev.choices.length < 8
          ? [...prev.choices, { content: "", isCorrect: false }]
          : prev.choices,
    }));
  };
  ...
  if (!isAuthenticated)
    return <div className="p-4">Please login as admin.</div>;
      ...
      <div className="p-4 border rounded-lg space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Select
              value={form.presentationId ? form.presentationId : "__none__"}
              onValueChange={(v) => {
            ...
            {form.presentationId &&
              (() => {
                const p = (presentations || []).find(
                  (x: any) => x.id === form.presentationId,
                );
                ...
                return specs.length ? (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {specs.map((s: string) => (
                      <span
                        key={s}
                        className="text-xs px-2 py-0.5 rounded border bg-yellow-50 border-yellow-200 text-gray-700"
                      >
                        {s}
                      </span>
                    ))}
            ...
            <div key={idx} className="flex items-center gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    choices: form.choices.map((cc, i) =>
                      i === idx ? { ...cc, content: e.target.value } : cc,
                    ),
                  })
                }
              ...
              {form.choices.length > 2 && (
                <Button variant="outline" onClick={() => removeChoice(idx)}>
                  Remove
                </Button>
              )}
            </div>
        ...
        <div className="space-y-2 mt-4">
          ...
          <div className="text-xs text-gray-600">
            Provide phrases to highlight in the linked PDF after the question is
            answered. Occurrence index = which appearance of the phrase on that
            page (1 = first, 2 = second, etc.).
          </div>

          {!form.presentationId ? (
            <div className="text-xs text-gray-600">
              Select a presentation to enable PDF highlights.
            </div>
          ) : pdfLoading ? (
            <div className="text-xs text-gray-600">Loading PDF…</div>
          ) : !pdfUrl ? (
            <div className="text-xs text-red-600">
              No PDF found for the selected presentation. Highlights are
              disabled.
            </div>
          ) : null}
          ...
          <div className="space-y-2">
              ...
              >
                <div className="sm:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setForm((prev) => ({
                        ...prev,
                        highlights: prev.highlights.map((hh, i) =>
                          i === idx ? { ...hh, page: val } : hh,
                        ),
                      }));
                    }}
                  />
                ...
                <div className="sm:col-span-2">
                  ...
                  <Input
                    type="number"
                    min={1}
                    value={h.occurrence ?? 1}
                    onChange={(e) => {
                      const val = Number(e.target.value);
                      setForm((prev) => ({
                        ...prev,
                        highlights: prev.highlights.map((hh, i) =>
                          i === idx ? { ...hh, occurrence: val } : hh,
                        ),
                      }));
                    }}
                  />
            ...
            <Button
              ...
              onClick={() =>
                setForm((prev) => ({
                  ...
                  highlights: [
                    ...prev.highlights,
                    { page: 1, phrase: "", occurrence: 1, color: "#fff59d" },
                  ],
                }))
              }
              disabled={!pdfUrl}
            ...
            {!pdfUrl && form.presentationId && !pdfLoading && (
              <div className="text-xs text-gray-600">
                Attach a PDF to this presentation to enable highlights.
              </div>
            )}
          </div>
          ...
          <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  {(form.highlights || []).map((h, i) => (
                    <li key={i} className="flex items-center gap-2">
                      <span
                        ...
                        style={{ backgroundColor: h.color || "#fff59d" }}
                      />
                      <span>
                        Page {h.page}: “{h.phrase}”{" "}
                        {h.occurrence ? `(occ ${h.occurrence})` : ""}
                      </span>
                    </li>
                  ))}
                </ul>
            ...
            <div className="border rounded overflow-hidden">
              <div className="px-3 py-2 bg-gray-50 border-b text-xs flex items-center justify-between">
                ...
                {pdfUrl && (
                  <a
                    className="text-ucla-blue underline"
                    href={pdfUrl}
                    target="_blank"
                    rel="noreferrer"
                  >
        ...
        <div className="flex justify-between">
          ...
          <Button
            ...
            disabled={
              ...
              !form.presentationId ||
              !form.prompt.trim() ||
              form.choices.filter((c) => c.content.trim()).length < 2 ||
              !form.choices.some((c) => c.isCorrect)
            }
          >
            {editingId
              ? updateMutation.isPending
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          ...
          <div className="w-72">
            ...
            <Select
              ...
              onValueChange={(v) => setFilterPresentation(v)}
            >
              <SelectTrigger>
                ...
                {g.items.map((q: any) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-600">
                        {(q as any).status || "approved"} • {q.choices.length}{" "}
                        choices
                      </div>
                    </div>
                    <div className="flex gap-2">
                      {(user?.role !== "user" ||
                        ...
                        <Button variant="outline" onClick={() => handleEdit(q)}>
                          Edit
                        </Button>
                      )}
                      {user?.role !== "user" && (
