...
export default function PresentationFilesViewer({
  isOpen,
  onClose,
  presentationId,
  title,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        className={`${isFullscreen ? "w-screen h-screen max-w-screen max-h-screen rounded-none p-0" : "sm:max-w-6xl w-[96vw] h-[88vh] p-0"} overflow-hidden flex flex-col gap-0`}
      >
        <DialogHeader
        ...
        <div className="flex-1 bg-muted relative">
          {loading ? (
            ...
            <div
              className={`grid gap-2 h-full ${hasPdf && hasPpt ? (isFullscreen ? "grid-cols-1 md:grid-cols-5" : "grid-cols-1 md:grid-cols-2") : "grid-cols-1"}`}
            >
              {hasPpt && (
                <div
                  className={`flex flex-col h-full ${hasPdf && hasPpt && isFullscreen ? "md:col-span-3" : ""}`}
                >
                  <div className="px-3 py-1 bg-background border-b border-border text-xs font-medium flex items-center gap-2">
                  ...
                  {pptUrl!.startsWith("blob:") ? (
                    <div className="flex-1 flex items-center justify-center bg-background">
                      <div className="text-center text-foreground">
                        ...
                        <Button onClick={() => window.open(pptUrl!, "_blank")}>
                          Open Presentation
                        </Button>
                      </div>
                    </div>
              ...
              {hasPdf && (
                <div
                  className={`flex flex-col h-full ${hasPdf && hasPpt && isFullscreen ? "md:col-span-2" : ""}`}
                >
                  <div className="px-3 py-1 bg-background border-b border-border text-xs font-medium flex items-center gap-2">
